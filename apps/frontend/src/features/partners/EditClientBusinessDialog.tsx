...
type Props = {
  open: boolean;
  business: PartnerClientBusiness | null;
  onClose: () => void;
  onValidateGSTIN: (gstin: string, excludeBusinessId: string) => Promise<{ exists: boolean; businessId?: string; businessName?: string }>;
  onSubmit: (input: {
    businessName: string;
    gstin?: string;
...
export function EditClientBusinessDialog({ open, business, onClose, onValidateGSTIN, onSubmit }: Props) {
    ...
    setForm({
      ...
      contacts:
        business.contacts.length > 0
          ? business.contacts.map((contact) => ({ name: contact.name, phone: contact.phone }))
          : [{ name: "", phone: "" }],
    });
    setSaving(false);
    setBusinessNameError("");
          ...
          <div className="px-5 py-5 sm:px-6">
            <div className="flex items-center justify-between gap-4">
              <div className="min-w-0">
                <h2 className="text-[28px] font-semibold tracking-tight text-slate-950">Edit Client</h2>
              </div>
              <Button
                type="button"
                variant="ghost"
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(event) => {
                        ...
                        void onValidateGSTIN(trimmed, business.id).then(setGSTINValidation);
                      }}
                      onBlur={() => {
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    onClose();
                    setStatusDialog({ tone: "success", title: "Client updated successfully" });
                  } catch (err) {
                    setStatusDialog({
                      tone: "error",
